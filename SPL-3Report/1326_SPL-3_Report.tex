...
This chapter presents representative test cases for \textbf{InnerVoice AI}, covering unit tests for NLP services, component tests for data models, API integration tests, and end-to-end integration tests. One or two key test cases are selected from each segment to demonstrate coverage.

\section{Unit Test Cases}

\textbf{Test Case 1:}\\
\textbf{Test Scenario:} Detect Bangla Unicode text using the Language Detection Service.\\
\textbf{Steps:}
\begin{enumerate}
    \item Instantiate the \texttt{LanguageDetector} class.
    \item Call \texttt{detect\_language('আমি খুব খুশি আজকে')}.
    \item Observe the returned language code.
\end{enumerate}
\textbf{Expected Outcome:} The method returns \texttt{language\_code = 'bn'}.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 2:}\\
\textbf{Test Scenario:} Classify positive English text using the Sentiment Analysis Service.\\
\textbf{Steps:}
\begin{enumerate}
    \item Instantiate the \texttt{SentimentAnalyzer}.
    \item Call \texttt{analyze('I love this amazing wonderful day!')}.
    \item Check the returned label and verify the probability scores sum to approximately 1.0.
\end{enumerate}
\textbf{Expected Outcome:} The returned label is \texttt{'positive'} and the sum of all scores is approximately 1.0.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 3:}\\
\textbf{Test Scenario:} Detect joy as the dominant emotion for happy text using the Emotion Detection Service.\\
\textbf{Steps:}
\begin{enumerate}
    \item Instantiate the \texttt{EmotionDetector}.
    \item Call \texttt{detect('I am so incredibly happy and excited!')}.
    \item Verify the result contains all six emotion keys (\texttt{joy}, \texttt{sadness}, \texttt{anger}, \texttt{fear}, \texttt{surprise}, \texttt{neutral}) and that \texttt{joy > sadness}.
\end{enumerate}
\textbf{Expected Outcome:} All six emotion keys are present and the joy score is greater than the sadness score.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 4:}\\
\textbf{Test Scenario:} Verify OCEAN personality traits output and score range using the Personality Analysis Service.\\
\textbf{Steps:}
\begin{enumerate}
    \item Instantiate the \texttt{PersonalityAnalyzer}.
    \item Call \texttt{analyze('I love exploring new ideas and meeting people')}.
    \item Check that the result contains a \texttt{traits} dictionary with all five OCEAN keys and that every score is within [0.0, 1.0].
\end{enumerate}
\textbf{Expected Outcome:} The \texttt{traits} dictionary contains \texttt{openness}, \texttt{conscientiousness}, \texttt{extraversion}, \texttt{agreeableness}, and \texttt{neuroticism}, each with a score between 0.0 and 1.0.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 5:}\\
\textbf{Test Scenario:} Verify Banglish text detection and normalization using the Bangla Processor Service.\\
\textbf{Steps:}
\begin{enumerate}
    \item Instantiate the \texttt{BanglaProcessor}.
    \item Call \texttt{is\_bangla\_or\_banglish('ami valo achi')} and verify it returns \texttt{True}.
    \item Call \texttt{\_reduce\_repeated\_chars('daruuuuun')} and verify it returns \texttt{'daruun'}.
\end{enumerate}
\textbf{Expected Outcome:} Banglish text is correctly detected as Bangla, and repeated characters are normalized.\\
\textbf{Result:} Passed

\section{Component Test Cases --- Data Models}

\textbf{Test Case 6:}\\
\textbf{Test Scenario:} Verify Post model default values and UUID primary key.\\
\textbf{Steps:}
\begin{enumerate}
    \item Create a test user.
    \item Create a \texttt{Post} with \texttt{text='Hello world'} and no other fields.
    \item Check that the defaults are \texttt{language='en'}, \texttt{detected\_tone='friendly'}, and that \texttt{post.id} is a valid UUID.
\end{enumerate}
\textbf{Expected Outcome:} The Post is created with correct defaults and a UUID primary key.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 7:}\\
\textbf{Test Scenario:} Verify cascade deletion removes all related records when a user is deleted.\\
\textbf{Steps:}
\begin{enumerate}
    \item Create a test user and a Post for that user.
    \item Delete the user from the database.
    \item Count remaining Post records.
\end{enumerate}
\textbf{Expected Outcome:} \texttt{Post.objects.count()} equals 0 after user deletion.\\
\textbf{Result:} Passed

\section{API Integration Test Cases}

\textbf{Test Case 8:}\\
\textbf{Test Scenario:} Successful user registration and duplicate username rejection via the Authentication API.\\
\textbf{Steps:}
\begin{enumerate}
    \item Send a \texttt{POST} to \texttt{/api/auth/register/} with valid \texttt{username}, \texttt{email}, and \texttt{password}. Verify HTTP 201 with a \texttt{token} in the response.
    \item Send another \texttt{POST} to \texttt{/api/auth/register/} with the same \texttt{username}. Verify HTTP 400 with ``Username already exists''.
\end{enumerate}
\textbf{Expected Outcome:} First registration succeeds (201); second registration is rejected (400).\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 9:}\\
\textbf{Test Scenario:} Successful Bangla text analysis with input validation via the Text Analysis API.\\
\textbf{Steps:}
\begin{enumerate}
    \item Authenticate as a valid user.
    \item Send a \texttt{POST} to \texttt{/api/analyze/} with \texttt{text=''} (empty). Verify HTTP 400.
    \item Send a \texttt{POST} to \texttt{/api/analyze/} with \texttt{text='আমি খুব খুশি আজকে!'} (services mocked). Verify HTTP 200 with \texttt{data} and \texttt{context\_analysis} keys in the response.
\end{enumerate}
\textbf{Expected Outcome:} Empty text is rejected with 400; valid Bangla text returns a successful analysis (200).\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 10:}\\
\textbf{Test Scenario:} Rewrite with valid \texttt{post\_id} and invalid \texttt{post\_id} via the Text Rewriting API.\\
\textbf{Steps:}
\begin{enumerate}
    \item Authenticate as a valid user. Create a Post and its Analysis record.
    \item Send a \texttt{POST} to \texttt{/api/rewrite/} with the valid \texttt{post\_id} (TextRewriter mocked). Verify HTTP 200.
    \item Send a \texttt{POST} to \texttt{/api/rewrite/} with a non-existent \texttt{post\_id} (all zeros). Verify HTTP 404.
\end{enumerate}
\textbf{Expected Outcome:} Valid \texttt{post\_id} returns a successful rewrite (200); invalid \texttt{post\_id} returns 404.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 11:}\\
\textbf{Test Scenario:} Health check endpoint is publicly accessible.\\
\textbf{Steps:}
\begin{enumerate}
    \item Send a \texttt{GET} request to \texttt{/api/health/} without any authentication token.
    \item Inspect the response status code and body.
\end{enumerate}
\textbf{Expected Outcome:} Response returns HTTP 200 with \texttt{\{success: true, message: 'API running'\}}.\\
\textbf{Result:} Passed

\section{End-to-End Integration Test Cases}

\textbf{Test Case 12:}\\
\textbf{Test Scenario:} Full user journey --- Registration $\rightarrow$ Login $\rightarrow$ Analyze $\rightarrow$ History retrieval.\\
\textbf{Steps:}
\begin{enumerate}
    \item Send a \texttt{POST} to \texttt{/api/auth/register/} with valid credentials. Verify HTTP 201 and extract the token.
    \item Send a \texttt{POST} to \texttt{/api/auth/login/} with the same credentials. Verify HTTP 200.
    \item Send a \texttt{POST} to \texttt{/api/analyze/} with Bangla text (services mocked). Extract the returned \texttt{post\_id}.
    \item Send a \texttt{GET} to \texttt{/api/history/}. Verify the history contains 1 record matching the analyzed post ID.
\end{enumerate}
\textbf{Expected Outcome:} All steps succeed. The history endpoint returns exactly 1 entry whose \texttt{id} matches the analyzed post.\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 13:}\\
\textbf{Test Scenario:} Cross-user data isolation --- one user cannot see another user's data.\\
\textbf{Steps:}
\begin{enumerate}
    \item Register and authenticate User A.
    \item User A creates a post via \texttt{/api/analyze/}.
    \item Register and authenticate User B.
    \item User B sends a \texttt{GET} request to \texttt{/api/history/}.
\end{enumerate}
\textbf{Expected Outcome:} User B's history response returns an empty list (cannot see User A's data).\\
\textbf{Result:} Passed

\vspace{0.5cm}

\textbf{Test Case 14:}\\
\textbf{Test Scenario:} Token invalidation after logout.\\
\textbf{Steps:}
\begin{enumerate}
    \item Authenticate as a user.
    \item Send a \texttt{GET} to \texttt{/api/profile/}. Verify HTTP 200.
    \item Send a \texttt{POST} to \texttt{/api/auth/logout/}. Verify success.
    \item Send a \texttt{GET} to \texttt{/api/profile/} again using the same token.
\end{enumerate}
\textbf{Expected Outcome:} The final request returns HTTP 401 Unauthorized (token is no longer valid).\\
\textbf{Result:} Passed
